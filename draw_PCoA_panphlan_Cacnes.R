...
UniFrac <- read.dist("binary_jaccard_panphlan_joined_tables_cacnes_w_ref.txt")
Meta <- read.tidy("sample_city_w_reference.txt")
...
PCoA$Clade <- factor(PCoA$Clade, levels = c("Denver","Hong Kong","London","New York", "Oslo","Stockholm","IA-1","IA-2","IA-3","IB-1","IB-2","IB-3","IC", "II","III"))

Plot <- ggplot(PCoA, aes(x = PCoA1, y = PCoA2, colour=Clade))

Plot <- Plot + geom_point(size=1.5)
Plot <- Plot + xlab(paste0("PCoA1 (Variance Explained: 18%)"))
Plot <- Plot + ylab(paste0("PCoA2 (Variance Explained: 5.7%)"))
Plot <- Plot + theme_classic()
#Plot <- Plot + scale_color_brewer(palette = "Set1")
Plot <- Plot + theme(axis.title=element_text(size=18, face="bold"))
Plot <- Plot + scale_color_manual(values=c("#cdc673","#8a2be2","#ee7600","#6e8b3d","#1874cd","#ee6363","black","#666666","#cdc9c9","#8b4500","#cd8162","#ffe4c4","#00ffff","#c1ffc1","#ff69b4"))
...
ggsave("panphlan_acnes_w_ref_curated_new.pdf",width=8,height=7,units="in",useDingbats=FALSE)
